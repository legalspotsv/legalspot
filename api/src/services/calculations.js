 ...
 */
export async function calculateClientPeriod(clientId, period) {
  ...
  const total_hours = Math.round(Number(consumption.total_hours) * 10) / 10;
  const total_consumed = Math.round(Number(consumption.total_consumed) * 100) / 100;
  ...
  const result = {
    ...
    hourly_rate: Number(client.hourly_rate),
    carry_forward_balance: Number(client.carry_forward_balance),
 ...
 */
export async function generateBillingRecord(clientId, period) {
  const summary = await calculateClientPeriod(clientId, period);
  if (!summary) return null;

  const [existing] = await sql`
    SELECT * FROM billing_records WHERE client_id = ${clientId} AND period = ${period}
  `;
  ...
  const [prevBilling] = await sql`
    ...
    WHERE client_id = ${clientId} AND period < ${period}
    ORDER BY period DESC LIMIT 1
  `;

  const prevAccumulated = prevBilling?.accumulated_total ? Number(prevBilling.accumulated_total) : 0;
  ...
  if (existing) {
    await sql`
      UPDATE billing_records SET
        fixed_fee = ${fixed_fee}, total_consumed = ${summary.consumo_usd}, excess_amount = ${excess_amount},
        total_invoiced = ${total_invoiced}, accumulated_total = ${accumulated_total}
    ...
    await sql`
      ...
      VALUES (${id}, ${clientId}, ${period}, ${fixed_fee}, ${summary.consumo_usd},
 ...
 */
export async function closePeriod(clientId, period) {
  const summary = await calculateClientPeriod(clientId, period);
  if (!summary) return null;

  const record = await generateBillingRecord(clientId, period);

  if (summary.has_fixed_fee) {
    const newBalance = Math.round((summary.carry_forward_balance + (summary.saldo_a_favor || 0)) * 100) / 100;
    await sql`UPDATE clients SET carry_forward_balance = ${newBalance}, updated_at = ${new Date().toISOString()} WHERE id = ${clientId}`;
  }
